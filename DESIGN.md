...
| 현재 위치 표시 | 화면 한 켠과 지도 패널에서 플레이어가 머무는 장소를 표시한다. | 잦은 장소 이동 중에도 현재 진행 위치를 명확히 인지하게 한다. | 조사 화면, 취조실, 지도 패널 | 사용자 결정 반영 |
...
- 2026-07-07: 사용자 결정 반영. 지도와 화면 상단에서 현재 장소를 알려주는 현재 위치 표시 기능을 추가 방향으로 기록.
