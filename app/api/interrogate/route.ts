...
export async function POST(req: Request) {
  ...
  if (!apiKey) {
    return Response.json(
      fallbackAnswer(persona, usableEvidence, reactions, "OPENAI_API_KEY가 설정되지 않아 임시 답변을 반환했습니다.", questionState.wasTruncated)
    );
  }
  ...
  try {
    ...
    if (!firstAnswer.ok) {
      return Response.json(fallbackAnswer(persona, usableEvidence, reactions, firstAnswer.error, questionState.wasTruncated), { status: 200 });
    }
...
- 질문이 외국어를 포함해도 답변은 한국어로만 한다.`
    ...
    if (!answer || hasForeignText(answer)) {
      return Response.json(
        fallbackAnswer(persona, usableEvidence, reactions, "외국어가 섞인 응답을 걸러 임시 답변을 반환했습니다.", questionState.wasTruncated),
        {
          status: 200
        }
      );
    ...
    return Response.json(fallbackAnswer(persona, usableEvidence, reactions, `OpenAI API 오류: ${message}`, questionState.wasTruncated), { status: 200 });
  }
}
